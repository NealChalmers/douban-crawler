...
for c in range(pages_start, pages_end):
    url = r'https://movie.douban.com/j/new_search_subjects?sort=T&range=0,10&tags=电影,{}&start={}'.format(year, c * 20)
    headers = getheaders()
    headers = {'User-Agent': headers}
    s_pro_ip = random.choice(proxies)
    s_proxy = {'http': proxies}
    page = requests.get(url=url, headers=headers, proxies=s_proxy).json()
    ...
    for r in pidx:  # 每次加载是20条，字典、列表、字符串的索引都是从0开始的，所以用range(20)；注意，网站标签，是从1开始的
        ...
        if (len(attrs_name) > 0):
            ...
            director_id_get = []
            for link in (attrs_name[0].find_all('a')):
                director_id_get.append(re.compile("/celebrity/(\d*)/").findall(link.get('href')))
            director_id.append(director_id_get)
        else:
        ...
        if (len(attrs_name) > 1):
            ...
            screenWriter_id_get = []
            for link in (attrs_name[1].find_all('a')):
                screenWriter_id_get.append(re.compile("/celebrity/(\d*)/").findall(link.get('href')))
            screenWriter_id.append(screenWriter_id_get)
        else:
        ...
        if (len(attrs_name) > 2):  # 主演
            ...
            actor_id_get = []
            for link in (attrs_name[2].find_all('a')):
                actor_id_get.append(re.compile("/celebrity/(\d*)/").findall(link.get('href')))
            actor_id.append(actor_id_get)
        else:
            movie_actor.append('')
            actor_id.append('')
